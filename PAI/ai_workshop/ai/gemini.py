...
class GeminiClient:
    ...
    def _init_model(self, api_key: str, model_name: str):
        ...
        print(f"[DEBUG] Gemini _init_model - API key length: {len(api_key)}, First 10 chars: {api_key[:10] if api_key else 'None'}")
        
        # Validate API key format (Gemini keys typically start with "AIza")
        if api_key and not api_key.startswith("AIza"):
            print(f"[WARNING] Gemini API key may be invalid - doesn't start with 'AIza'")
